...
* 1997 data is provided in a different format than other years. The only way
* I've managed to compile the data is through stata. See sgf1997.do in Data\SGF
* for more on how this was done.
...
set	s	Truncated indices in datasets;

$gdxin 'temp\gdx\SGF_2014_00A1.gdx'
$loaddc s

parameter	sgf2014(*,*,*,*,*,*,s), sgf2015(*,*,*,*,*,*,s);
alias (ii,jj,kk,i,j,k,*);
...
* 1997 is a bit different. Indices are already mapped in STATA.
...
sgf_('2014',states,sgfid) = sum(idmap(sgfid,sgfdata), sum((map1415(sgfdata,s),cmap(states,clabel)), sum((ii,jj,kk,i,j), sgf2014(clabel,ii,jj,kk,i,j,s))));
sgf_('2015',states,sgfid) = sum(idmap(sgfid,sgfdata), sum((map1415(sgfdata,s),cmap(states,clabel)), sum((ii,jj,kk,i,j), sgf2015(clabel,ii,jj,kk,i,j,s))));
...
loop(idsum(chk,sgfid,id), sgftot(chk,sgfid) = yes;);

parameter chksum;
chksum(yr,states,sgftot(chk,sgfid),"diff")$sum(idsum(chk,sgfid,id), sgf_(yr,states,id))
	= sgf_(yr,states,sgfid) - sum(idsum(chk,sgfid,id), sgf_(yr,states,id));
...
chkcat(ecat) = sum(states, sgf_('2014',states,ecat));
display chkcat;
